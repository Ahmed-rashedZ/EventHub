                ...
                ${ev.sponsors.map(sp => `
                   <div style="display:flex; align-items:center; gap:10px; background:rgba(255,255,255,0.04); padding:10px; border-radius:10px; border:1px solid rgba(255,255,255,0.05); cursor:pointer;" onclick="navigateToProfile(${sp.id})">
                      <div class="avatar" style="width:32px; height:32px; font-size:12px;">
                          ${sp.profile?.logo ? `<img src="${sp.profile.logo}" style="width:100%;height:100%;border-radius:50%;object-fit:cover;">` : (sp.name ? sp.name.charAt(0) : '?')}
                      </div>
                      <div style="flex:1">
...
.ed-content { position: relative; max-height:85vh; overflow-y:auto; }
.ed-banner {
