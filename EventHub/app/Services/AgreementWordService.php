...
class AgreementWordService
{
    ...
    {
        $sponsorshipRequest->load(['event.venue', 'manager', 'sponsor']);

        // Find the LAST UPLOADED file (not acceptance/revision records — only actual file uploads)
        ...
        $filename = 'agreements/agreement_' . $sponsorshipRequest->id . '_final.pdf';
        ...
        if ($lastVersion && \Illuminate\Support\Facades\Storage::disk('public')->exists($lastVersion->file_path)) {
            ...
            // If the user uploaded a PDF directly, just copy it to the final destination!
            if (strtolower(pathinfo($uploadedFilePath, PATHINFO_EXTENSION)) === 'pdf') {
                copy($uploadedFilePath, $fullPdfPath);
                $pdfGenerated = true;
            } else {
                // Generate a temporary VBScript to convert DOCX to PDF using Word COM Object
                $vbsScript = '
...
';
        ...
        if (!$pdfGenerated) {
            // If conversion failed but we have an uploaded file, just copy it as-is
            // (the downloadFinal method will serve the original uploaded file anyway)
            if ($lastVersion && \Illuminate\Support\Facades\Storage::disk('public')->exists($lastVersion->file_path)) {
                $uploadedFilePath = storage_path('app/public/' . $lastVersion->file_path);
                copy($uploadedFilePath, $fullPdfPath);
            } else {
                // Ultimate fallback: generate from Blade template (only if no uploaded file exists at all)
                $pdfData = [
                    'event'       => $sponsorshipRequest->event,
                    'sponsor'     => $sponsorshipRequest->sponsor,
                    'manager'     => $sponsorshipRequest->manager,
