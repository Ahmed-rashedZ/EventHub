...
| `index()` | GET | عرض طلبات الرعاية حسب الدور |
| `update($id)` | PUT | قبول/رفض طلب + ربط الراعي بالفعالية + إنشاء عقد PDF |
...
### 3. 🤝 نظام الرعاية ثنائي الاتجاه
- **Manager → Sponsor:** المدير يدعو راعي
- **Sponsor → Manager:** الراعي يتقدم لفعالية
- نظام القبول/الرفض حسب من بادر
- عند القبول: ربط في `event_sponsor` + إنشاء عقد PDF
- مستويات الرعاية: Diamond (لوغو) → Gold → Silver → Bronze
...
> 📝 **ملاحظة:** هذا الملف يمثل التوثيق الكامل للمشروع حتى تاريخ 2026-04-12. أي تعديلات مستقبلية يجب تحديث هذا الملف.
